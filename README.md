...
`apps/demo/terminal.html` is a terminal replay of five Jev decisions: what Jev is shown, each question and score, the latency, and the verdict, ending with the same force push judged wrong on one task and right on the next. `apps/demo/clip-reread.html` and `apps/demo/clip-routing.html` are the two short clips for posting: the same file read three times with and without Reflex, and Jev routing 38 of 39 steps to Haiku on the benchmark (`context.html` is the matching one-page summary). `apps/demo/economics.html` is the token-economics page: per-step input tokens from the real benchmark traces, the $1.75 to $0.28 result, and the projected bill for 100, 200 and 400-step sessions (`economics.png` beside it). `apps/demo/story.html` is the cinematic version: six beats, one tool call each, autoplaying and looping. `apps/demo/flow.html` plays the two sessions as a flow: packets are tool calls, the tank is what the model re-reads every step. `apps/demo/infographic.html` is the one-page summary (`infographic.png` beside it). `apps/demo/index.html` replays the real benchmark trace step by step.

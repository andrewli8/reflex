...
Install Reflex (npm package `agent-reflex`) in this project and verify it works. Reflex is an advisory hook layer for coding agents: it asks Jev, a small decision model, whether a destructive command belongs to my task and whether an edit breaks a constraint I stated, and sends the call to the permission prompt only when it does not. It also restates my constraints every turn, hands you a ledger after compaction, and trims oversized output. It does not replace permissions or sandboxing.

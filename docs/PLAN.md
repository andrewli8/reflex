...
### Levels, routing, and the edit-summary fix (2026-09-20)

One `level` knob (`off | watch | nudge | ask | auto | ultra`) applied as a preset before explicit config keys; `reflex level <name>` writes it and clears the keys it owns. `auto` and `ultra` set `askBecomesDeny`: a call that would prompt is denied with a reason, so unattended runs never hang on a permission prompt and never auto-approve a force push. `ultra` also enables per-step model routing in the AI SDK adapter: before each step Jev answers a `choice` (small: routine read/run/one-line edit; large: design, debug, multi-file change) and `prepareStep` swaps the model; only a confident `small` (≥ 0.6) routes down, anything else stays large. Claude Code and Codex hooks cannot change the model per call, so routing is AI SDK only.

While verifying levels through the real hook, the constraint check scored the Auth0→Clerk edit 0.21 instead of the 0.92–0.99 seen in the probes. Cause: write summaries carried only the path (`Edit src/auth0.config.ts`); the old/new strings never reached the model. Summaries now include a redacted 40-character old → new snippet (or the first 60 characters of written content). Re-run from a cold session with only the prompt: Auth0→Clerk 0.99 (asked), redirect fix 0.20 (executed). This is the first constraint catch produced end to end by the shipped hook rather than by a probe script.

Routing benchmark (2026-09-20), one repeat per arm, oracle-verified completion: Sonnet 5 alone 38 steps, 858,529 input tokens, $1.75; Sonnet 5 with `ultra` (collapse plus Jev routing) 39 steps, 38 on Haiku 4.5 and 1 on Sonnet, 263,126 input tokens, $0.28. Prices: Sonnet 5 $2/$10, Haiku 4.5 $1/$5 per million. Caveat: the task is a read-then-fix job with almost no reasoning steps, so Jev's routing down is correct but the benchmark does not test routing back up; a design-heavy task is needed before claiming a general number.


### Pattern hits judged in context (2026-09-20)

Design change after the founder's point that the host's allowlist already covers command-level rules: deterministic checks now produce candidates, and at `ask`/`auto`/`ultra` Jev judges destructive-pattern candidates against the task. Three phrasings probed on four contexts (asked-for, implied, forbidden, unrelated force push): the constraint boolean gave 0.54 / 0.61 / 0.98 / 0.87 (unusable, destructive prior), a "did the goal request it" boolean gave 0.91 / 0.35 / 0.02 / 0.03 (misses implied), and a four-way choice gave requested 0.75 / needed 0.70 / forbidden 0.99 / unrelated 0.97 (all correct). Shipped the choice with `patternIntended` = 0.6 on requested + needed. Verified through the real hook from cold sessions: asked-for and implied proceed with a note, forbidden and unrelated prompt. Model calls on reads are now off outside `ultra` (`modelOnReads`), since the read questions never changed a decision.

Near-miss replay with pattern judgment (2026-09-20): the historical 1,693-call session whose task was a features-and-specs discussion contained three `git checkout -- apps/brand && git clean -fd apps/brand` calls. Jev judged all three unrelated to the task (0.95, 0.97, 0.98), so they still prompt. That matches a human reading: deleting untracked work in a worktree was not part of a spec-writing task. No historical prompt was waved through; the four synthetic contexts remain the evidence that requested and needed cases proceed.


### Numbers refreshed on the current build (2026-09-20)

Replay of the last 20 non-temporary sessions, 3,606 calls, level `ask`, provider Jev: 0 skips (duplicate detection is deliberately conservative and, with the same-server and empty-result rules, no longer fires on these sessions), 22 asks (9 constraint conflicts, all in two sessions where the user said "stop loop" and the agent restarted it, plus 13 destructive patterns judged unrelated to their tasks), 0 false vetoes, 263 KB of 2,983 KB tool output trimmed, 6.7% of re-read-weighted tool-output tokens saved, 42% of results never referenced again. Trims: 47, of which 14 cut an identifier the agent later used; all 14 were results over 12 KB (huge file reads and command dumps), where the archive path in the stub is the recovery route. Read-class command output under 12 KB is no longer trimmed.

Fresh-install test: `npm pack`, global install into a throwaway prefix, `init`, `key`, `doctor`, a hook call through the shim in an empty environment (Jev reached, forbidden force push 0.99), `report`. It exposed a launch blocker: the CLI's main guard compared `argv[1]` to the module path and did nothing through the npm bin symlink. Fixed with a real-path comparison.
